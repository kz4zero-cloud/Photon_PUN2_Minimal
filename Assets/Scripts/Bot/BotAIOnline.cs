...
public class BotAIOnline : MonoBehaviourPunCallbacks
{
    ...
    PhotonView pv; Rigidbody rb; CapsuleCollider col;
    ...
    void EnableByRole()
    {
        // AIは Master or Offline のみ実行（他クライアントは描画だけ）
        enabled = PhotonNetwork.IsMasterClient || PhotonNetwork.OfflineMode;
        // 実際の挙動は FixedUpdate にあるので、enabled=false で停止
    }

    public override void OnEnable() { base.OnEnable(); EnableByRole(); }
    public override void OnJoinedRoom() { EnableByRole(); }
    public override void OnMasterClientSwitched(Photon.Realtime.Player newMasterClient) { EnableByRole(); }
